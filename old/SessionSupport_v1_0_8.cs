            ...
            while (!_stopping)
            {
                ...
                try
                {
                    pipe = new NamedPipeServerStream(
                        ...
                        4096,
                        4096,
                        PipeSecurityFactory_v1_0_2.Create());

                    Logger.Info("command server listening session={0}", _sessionName);
                    pipe.WaitForConnection();
                    Logger.Info("command server connected session={0}", _sessionName);
                    using (pipe)
